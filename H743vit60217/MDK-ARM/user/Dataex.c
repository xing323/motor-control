...
dt_flag_t f;                                   //发送结构体
UI_Flag UI;
Qiankui QK_M1, QK_M2, QK_M3, QK_M4, QK_M5, QK_M6;
...
extern uint8_t data_to_send[255];              //发送缓存数组
uint8_t data_to_send_s[100];
...
void Data_Exchange(void)
{
    ...
    static uint8_t  Encoder_cnt       = 5;          //编码器状态更新
    static uint8_t  Control_Inc_cnt   = 10;          //控制信号更新
    static uint8_t  send2NiMing_cnt   = 50;         //数据采集时间（发送到匿名上位机）
    ...
    if((angle_Clear_Flag == 1) || (UI.angle_Clear_Flag == 1))
    {
        ...
        if(angle_Clear_Finished_Flag)
        {
            angle_Clear_Flag = 0;    //若角度清零，清除角度清零使能信号
            UI.angle_Clear_Flag = 0;
        }
        //若角度清零，角度清零完成标志置1
    }
...
void Send_Data2NiMing(void)
{
    ...
    //	temp = Motor4.PWM;    //发送电机跟踪误差
    //	data_to_send[_cnt++]=BYTE1(temp);
    //	data_to_send[_cnt++]=BYTE0(temp);
    //
    //	temp = Motor6.PWM;    //发送电机跟踪误差
    //	data_to_send[_cnt++]=BYTE1(temp);
    //	data_to_send[_cnt++]=BYTE0(temp);
...
void Current_updata(void)
{
    ...
    Motor1.curent = Motor1.curent * 0.7 + (((float)Data[0] / 65536) * Vrefint + 0.005   - 0.5 * Vrefint) * 0.3 * 1.7857f;
    Motor2.curent = Motor2.curent * 0.7 + (((float)Data[1] / 65536) * Vrefint + 0.0058  - 0.5 * Vrefint) * 0.3 * 1.7857f;
    Motor3.curent = Motor3.curent * 0.7 + (((float)Data[2] / 65536) * Vrefint + 0.0044  - 0.5 * Vrefint) * 0.3 * 1.7857f;
    Motor4.curent = Motor4.curent * 0.7 + (((float)Data[3] / 65536) * Vrefint + 0.0062  - 0.5 * Vrefint) * 0.3 * 1.7857f;
    Motor5.curent = Motor5.curent * 0.7 + (((float)Data[4] / 65536) * Vrefint + 0.005   - 0.5 * Vrefint) * 0.3 * 1.7857f;
    Motor6.curent = Motor6.curent * 0.7 + (((float)Data[5] / 65536) * Vrefint + 0.0055  - 0.5 * Vrefint) * 0.3 * 1.7857f;

    //记录电流限幅时刻的PWM（最大电流为2.9A）
    if((Motor1.curent > 2.8) || (Motor1.curent < -2.8)) Motor1.I_lim_PWM =  Motor1.PWM;
    else Motor1.I_lim_PWM = 11999;
    if((Motor2.curent > 2.8) || (Motor2.curent < -2.8)) Motor2.I_lim_PWM =  Motor2.PWM;
    else Motor2.I_lim_PWM = 11999;
    if((Motor3.curent > 2.8) || (Motor3.curent < -2.8)) Motor3.I_lim_PWM =  Motor3.PWM;
    else Motor3.I_lim_PWM = 11999;
    if((Motor4.curent > 2.8) || (Motor4.curent < -2.8)) Motor4.I_lim_PWM =  Motor4.PWM;
    else Motor4.I_lim_PWM = 11999;
    if((Motor5.curent > 2.8) || (Motor5.curent < -2.8)) Motor5.I_lim_PWM =  Motor5.PWM;
    else Motor5.I_lim_PWM = 11999;
    if((Motor6.curent > 2.8) || (Motor6.curent < -2.8)) Motor6.I_lim_PWM =  Motor6.PWM;
    else Motor6.I_lim_PWM = 11999;
...
void Check_Back(void)
{
    ...
    data_to_send_s[_cnt++] = 0xAA;
    data_to_send_s[_cnt++] = 0xAA;
    data_to_send_s[_cnt++] = 0xF3;  //功能字，发送之后返回校验
    data_to_send_s[_cnt++] = 0;

    data_to_send_s[_cnt++] = Position_zeroed_Flag;
    data_to_send_s[_cnt++] = UI.angle_Clear_Flag ;
    data_to_send_s[_cnt++] = UI.ShouDongKangFu_mode;
    data_to_send_s[_cnt++] = UI.AutoKangFu_mode;

    data_to_send_s[_cnt++] = UI.ShouDongKangFu_SpeedSet_Flag;
    data_to_send_s[_cnt++] = UI.AutoKangFu_CiShu;
    data_to_send_s[_cnt++] = UI.Curve_Type;
    data_to_send_s[_cnt++] = UI.Para_Send_Flag;
    ...
    data_to_send_s[_cnt++] = Motor1.Enable_Flag;
    data_to_send_s[_cnt++] = Motor2.Enable_Flag;
    data_to_send_s[_cnt++] = Motor3.Enable_Flag;
    data_to_send_s[_cnt++] = Motor4.Enable_Flag;

    data_to_send_s[_cnt++] = Motor5.Enable_Flag;
    data_to_send_s[_cnt++] = Motor6.Enable_Flag;
    data_to_send_s[_cnt++] = LeftLeg_NeiWaiXuan ;
    data_to_send_s[_cnt++] = RightLeg_NeiWaiXuan;

    data_to_send_s[3] = _cnt - 4;


    for(i = 0; i < _cnt; i++)
        sum += data_to_send_s[i];
    data_to_send_s[_cnt++] = sum;

    HAL_UART_Transmit(&huart1, (uint8_t *)&data_to_send_s, _cnt, 100);
...
{
    data_to_send_s[0] = 0xAA;
    data_to_send_s[1] = 0xAA;
    data_to_send_s[2] = 0xEF;
    data_to_send_s[3] = 2;
    data_to_send_s[4] = head;
    data_to_send_s[5] = check_sum;

    uint8_t sum = 0;
    for(uint8_t i = 0; i < 6; i++)
        sum += data_to_send_s[i];
    data_to_send_s[6] = sum;

    HAL_UART_Transmit(&huart1, (uint8_t *)&data_to_send_s, 7, 10);
}
...
void Send_Data2New_NiMing(void)
{
    ...
    data_to_send[_cnt++] = 0x50;  //源地址（单片机地址）  0x50
    data_to_send[_cnt++] = 0xFE;  //目标地址（发给上位机）0x01
    data_to_send[_cnt++] = 0xF1;  //功能字，F1为用户数据
    data_to_send[_cnt++] = 0;
    data_to_send[_cnt++] = 0;     //数据长度2byte
...
//    data_to_send[_cnt++] = BYTE3(f_temp);
    ...
    //	data_to_send[_cnt++]=BYTE0(f_temp);
		...
		flen = _cnt - 6;	
    data_to_send[4] = BYTE0(flen);
		data_to_send[5] = BYTE1(flen);
    ...
    data_to_send[_cnt++] = 0xAB;  //帧头
    data_to_send[_cnt++] = 0x50;  //源地址（单片机地址）  0x02
    data_to_send[_cnt++] = 0xFE;  //目标地址（发给上位机）0x01
    data_to_send[_cnt++] = 0xF2;  //功能字，F2为用户电流数据
    ...
    //数据长度
		flen = _cnt - 6 -i;
		data_to_send[i+4] = BYTE0(flen);
    data_to_send[i+5] = BYTE1(flen);
